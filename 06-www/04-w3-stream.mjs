...
server.on("request", async (req, res) => {
    ...
    // Event listener per il flusso di dati: invia ogni chunk (pezzo) di dati al client
    fileStream.on("data", (chunk) => res.write(chunk));

    // Event listener per la fine del flusso: chiude la risposta una volta inviato tutto il contenuto
    fileStream.on("end", () => res.end());

    // Event listener per eventuali errori nel flusso
