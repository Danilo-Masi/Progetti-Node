...
import getFeedEntries, { getLatestEntries } from "./get-feeds.mjs";
    ...
    wrap(async (req, res) => {
        ...
        try {
            ...
            if (reason) {
                ...
                req.log.error("An error occured during Source.save()");
                res.status(500).send();
            }
        }
    })
...
app.delete(
    ...
    wrap(async (req, res) => {
        try {
            ...
            req.log.error(`Error while deleting Source with id ${req.params.id}`);
        }
    })
)
